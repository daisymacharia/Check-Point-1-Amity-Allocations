...
class TestPeople(unittest.TestCase):
    ...
    def test_fellow_is_allocated_a_living_space(self):
        ...
        message = sys.stdout.getvalue().strip()
        self.assertIn('Ivy successfully added to living space test_livingspace', message)
    ...
    def test_reallocation_of_fellow_to_new_office(self):
        ...
        new_office = self.amity.create_room('OFFICE', 'test_office_reallocations')
    ...
    def test_reallocation_of_fellow_to_new_living_space(self):
        """Tests that a fellow is successfully reallocated to a new living space"""
        new_livingspace = self.amity.create_room('LIVINGSPACE', 'test_livingspace_reallocations')
    ...
    def test_reallocation_of_staff_to_new_office(self):
        ...
        new_office = self.amity.create_room('OFFICE', 'test_office_reallocations_for_staff')
        self.amity.reallocate_staff('test_staff', 'test_office_reallocations_for_staff' )
        message = sys.stdout.getvalue().strip()
    ...
    def test_reallocated_fellow_is_removed_from_previous_office(self):
        """Tests that a fellow is removed from the previous room once they are reallocated to a new office"""
        #add fellow test_fellow_allocations who is allocated an office
        self.amity.add_person('FELLOW', 'test_fellow_reallocation')
        new_office = self.amity.create_room('OFFICE', 'test_office_reallocations')
        self.amity.reallocate_fellow('test_fellow_reallocation', 'test_office_reallocations' )
        message = sys.stdout.getvalue().strip()
        self.assertIn('Fellow sucessfully removed from previous office', message)


    def test_reallocated_fellow_is_removed_from_previous_livingspace(self):
        """Tests that a fellow is removed from the previous livingspace once they are reallocated to a new office"""
        #add staff test_staff_allocations who is allocated an office
        self.amity.add_person('FELLOW', 'test_fellow_reallocation')
        new_livingspace = self.amity.create_room('LIVINGSPACE', 'test_livingspace_removal_of_staff')
        self.amity.reallocate_fellow('test_fellow_reallocation', 'test_livingspace_removal_of_staff' )
        message = sys.stdout.getvalue().strip()
        self.assertIn('Fellow sucessfully removed from previous livingspace', message)

    def test_reallocated_staff_is_removed_from_previous_office(self):
        """Tests that a staff is removed from the previous office once they are reallocated to a new office"""
        #add staff test_staff_allocations who is allocated an office
        self.amity.add_person('STAFF', 'test_staff_reallocation')
        new_office = self.amity.create_room('OFFICE', 'test_office_removal_of_staff')
        self.amity.reallocate_staff('test_staff_reallocation', 'test_office_removal_of_staff' )
        message = sys.stdout.getvalue().strip()
        self.assertIn('Staff sucessfully removed from previous office', message)

    def test_cannot_reallocated_staff_to_a_livingspace(self):
        """Tests that a staff is not reallocated to a living space"""
        new_office = self.amity.create_room('LIVINGSPACE', 'test_livingspace_reallocations_for_staff')
        self.amity.reallocate_staff('test_staff', 'test_livingspace_reallocations_for_staff' )
        message = sys.stdout.getvalue().strip()
    ...
    def test_removes_people_from_waiting_list_after_allocation(self):
        """Tests that person either fellow or staff is removed from the waiting list once reallocated
         to a new room  """
